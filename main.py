...
# Changed the characteristic data of the dataset to the numeric data format
...
def Test_Relation(dataset):
    dataset = dataset[['Time spent on Online Class', 'Rating of Online Class experience', 'Time spent on self study',
    ...
    x2 = df[
        ['Time spent on self study', 'Time spent on Online Class', 'Time spent on social media', 'Time spent on TV']]
    y = df[['Rating of Online Class experience']]

    # Model training -> train size = 80%, test size = 20%
    ...
    # The number of all class is plotted with subplot
    plt.subplot(2, 3, 1)
